...
- live preflight for `src/code-graph/commands.ts`: 340ms total after compact validation-command filtering
...
- compact validation commands dropped from 114 to 11, with `limits.validationCommands: 20` and `omissions.validationCommands: 103`, led by `bun test ./src/code-graph/__tests__/builder.test.ts`, `bun test ./src/code-graph/__tests__/commands.test.ts`, and `bun test ./src/code-graph`
